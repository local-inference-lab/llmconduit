...
Rust LLM API gateway. Accepts OpenAI Responses, OpenAI Chat Completions, and Anthropic Messages on the front; forwards to OpenAI-compatible `/v1/chat/completions` upstream, and proxies legacy `/v1/completions`. Adds server-side Brave web search, per-model defaults, profile-driven routing with failover across upstreams, replay caching, request-log analysis, and an optional debug UI.
...
- **Only a profile's own `fallbacks:` are failure fallbacks.** `ProfileRoutingClient::chain_for` builds the failover chain from the resolved profile's primary `upstream` plus its declared `fallbacks:` only. Never fail over to some other `upstreams:` entry just because it exists - only a chain step the matched profile actually names is a failover candidate.
...
- **Inbound `Content-Encoding` is decoded in `log_api_call`.** `zstd`/`gzip`/`deflate`/`br` request bodies are decompressed BEFORE the `Json`/`Bytes` extractors run (codex-tui 0.145+ ships `Content-Encoding: zstd` via `enable_request_compression`). The `Content-Encoding` header is stripped and `Content-Length` is corrected on the rebuilt request so the upstream (which builds its own request from the parsed body) never sees a stale encoding. Unknown encodings return `415`, not a misleading 400 JSON-parse error. Bodies are fully buffered, so decode is synchronous in place (`zstd`/`flate2`/`brotli` crates) — do not add `async-compression`.
- **`/v1/responses` also serves a WebSocket upgrade (GET).** codex-tui's `openai-beta: responses_websockets=2026-02-06` beta attempts a WS upgrade FIRST and falls back to HTTPS POST on any non-101. The handler accepts one inbound frame (text=JSON or binary=zstd-compressed JSON, the codex `enable_request_compression` path) and streams `response.*` events as text frames — the SAME `SseEvent` data the HTTP SSE path emits (via `responses_wire_event_data`), terminating on `response.completed`/`failed`/`incomplete`/`error` and closing the socket. It reuses `stream_responses_with_api_call_id` with `api_call_id=None`. A plain (non-upgrade) GET returns `426 Upgrade Required`. The WS protocol is reverse-engineered from the codex binary (the beta is undocumented) — codex's `Option<WebSocketUpgrade>`-style fallback means a wrong-but-upgrading server stops codex's fallback, so validate against a real codex session before changing the frame shape. WS turns are NOT in the FlowStore/turn-capture (those gate on POST).
...
- **Image handling is per-profile opt-in.** A profile with no `image_analysis` is native passthrough by design: its images reach the upstream untouched. When the resolved profile sets `image_analysis`, `activate_image_agent` strips the latest turn's images to placeholders and hands them to the `analyzeImage` tool; a role-agnostic residual-image pass (`engine.rs`, gated on that active vision session) then sweeps every `ResponseItem` (not just `role=="user"`) for `ContentItem::InputImage` (`image_url` or `file_id`) the strip missed and either degrades it to a text placeholder (`image_analysis.residual_images: placeholder`, default) or rejects the turn pre-dispatch with a 4xx (`reject`) - never forwarded raw. A degraded turn forces `request.store = false` to bypass the replay cache both ways (lookup + store), since two different images collapse to byte-identical placeholder text at the same position.

## Config resolution order

Global → matched model profile templates (`extends:` in order) → matched model profile → explicit request fields. Profile-root shorthand keys merge into `upstream_chat_kwargs` via custom `Deserialize` (`config.rs:867-939`); explicit `upstream_chat_kwargs:` wrapper overrides shorthand on conflict.

`Config::resolve_route` picks exactly ONE profile per request: an exact key (trimmed, case-insensitive) beats any glob regardless of declaration order; among globs, the first declared match wins. That single profile's fields (already `extends`-merged at config load) apply wholesale - there is no secondary match against the resolved/served model or a reserved fallback profile. For `system_prompt_prefix`, the global prefix is prepended and the resolved profile's prefix (if any) is appended before request `instructions`.

Routing is always on: every `model_profiles` entry names an `upstream:`, validated against `upstreams:` at startup (`validate_model_profiles`, `config.rs`). `/v1/models` is built entirely from the exact (non-glob) profile keys, in declaration order, and never proxies to an upstream; `context_length` is filled in per entry when the served model's catalog metadata is known. A request model with no matching profile is a 404; a blank/missing model is a 400 unless a glob profile matches the empty string and also sets `upstream_model`.
...
- An `upstreams:` entry authenticates only with its own `api_key`; there is no implicit environment fallback (`OPENAI_API_KEY` is not read). The one env path is explicit per entry: `api_key_env: VAR`, resolved once at startup, mutually exclusive with `api_key`, and a hard startup error when the variable is unset or blank.
...
- `/v1/completions` is a raw upstream proxy with header filtering that never goes through the engine (no shaping, no replay, no instrumentation). It still resolves the request body `model` through the same profile chain (`ProfileRoutingClient::chain_for`) used by the engine routes, rewriting `model` to a fallback hop's remap when one is set.
...
- `ProfileRoutingClient` (the always-on upstream client, `upstream.rs:2062`) caches the per-provider `/v1/models` catalog for 300s (`ROUTING_MODEL_CATALOG_TTL_SECS`, `upstream.rs:382`); `/v1/models` and per-request context budgeting share this cache. Tests that change provider catalogs need a fresh client or must account for the cache.
- `Gateway`'s own `UpstreamModelCatalog` (`engine.rs`, `UPSTREAM_MODEL_CATALOG_TTL_SECS=300s`) is a separate cache used only as the non-routing context-budgeting fallback when the routing layer's `BackendCandidatePlan` has no known limit for a chain step; it does not back `/v1/models`. Tests that depend on catalog changes need to construct a fresh `Gateway`.
...
- Anthropic `/v1/messages` stream-shape invariants (one terminal `message_delta`, signed `thinking` blocks, etc.) are asserted by `src/adapters/responses_to_anthropic/conformance.rs` — `pub`, not `#[cfg(test)]`, so integration crates reach it at `llmconduit::adapters::responses_to_anthropic::conformance::{assert_stream_conformant, assert_sse_conformant, Surface}`. A `thinking` block's `signature_delta` is the real upstream one when present, else a synthetic `llmconduit-synthetic-v1:<sha256>` marker (`SYNTHETIC_SIGNATURE_PREFIX`, `models/anthropic.rs`) — stripped on Anthropic ingress (`anthropic_to_responses.rs`) so a client echo-back is never replayed upstream as a genuine signature.
...
- `LLMCONDUIT_BUILD_DASHBOARD` is a BUILD-time env var (build.rs runs `npm run build` + embeds via `include_dir!`); without it `cargo build` still succeeds on a node-less host (a stub shell is embedded). Docker builds the frontend in a separate Node stage and passes its output through the mutually exclusive `LLMCONDUIT_DASHBOARD_DIST` build-time path, keeping Node out of the final image. Runtime gating is the existing `--with-debug-ui`.
...
- Durable turn capture (F1): `turn_capture_dir` arms an opt-in, on-disk, per-turn diagnostic on its OWN gate — INDEPENDENT of `--with-debug-ui` — over the SAME `/v1/responses`,`/v1/messages`,`/v1/chat/completions` whitelist. It is a REDACTED, BOUNDED-MEMORY surface: request bodies route through the SAME secret + image-URI redactors as the `body_payload` log (a new logged surface must not bypass `redact_payload_secrets` — line 137), and each section (`inbound_request`/`upstream_request`/`upstream_response`/`served_response`) STREAMS to a per-turn temp file under `<turn_capture_dir>/.work/<api_call_id>/` (never a full body in RAM, never a `Bytes` slice of the 256 MiB middleware buffer — line 144). At the both-`done` barrier (engine terminal + served-body tee) the sections assemble by streaming-escape into an ATOMIC single JSON `<turn_capture_dir>/<api_call_id>.json` (tmp → fsync → rename), keyed by `api_call_id`, then the `.work/<id>/` dir is best-effort deleted. Age-rotated via `debug_log_max_age_hours` — `debug_log_dirs()` includes `turn_capture_dir`, so `log_rotation::cleanup_dump_files` prunes the `<id>.json` and `cleanup_orphan_work_dirs` sweeps crash-orphaned `.work/<id>/` in the same startup pass. `served_response`/`upstream_response` are raw model OUTPUT (streamed, not secret-scanned beyond image-URI redaction on the failed-body path). The disabled sink (no `turn_capture_dir`) is a zero-op — no thread, no allocation, no fs.
...
- Dashboard (gap 16): the control-room OVERVIEW is the 5th hash route `#/overview` (`views/overview/OverviewView.tsx`) — a THIN composer that COMPOSES the gap-01–15 surfaces; it adds NO backend seam and re-fetches nothing. Routing slots in via `router/useHashRoute.ts` (`RouteName`/`ROUTES`), `views/registry.ts`, `NavTabs.tsx` LABELS, and the e2e `harness.ts` `VIEWS` (so the per-view screenshot loop covers it — its `overview-chromium-linux.png` baseline is a real view baseline). NEW pure model `views/overview/overviewModel.ts` (DOM-free, sibling of `failureTaxonomy.ts`): `topByVolume`/`topByCost(rows, 'model'|'provider')` (cost inherits the WEAKEST `cost_confidence` of contributing PRICED flows — mixed confident+estimated ⇒ estimated, never upgraded; no priced flow ⇒ `—`, never `$0.00`; `topByCost` ranks only priced groups) + `tokenMix(rows)` (unreported optional cached/reasoning ⇒ `—`, never a fabricated `0`). WIRE SOURCE (the recurring trap — honored): per-provider latency/error tiles read the gap-12 `ProviderLatency` from the REST `/topology`+`/snapshot` node via the gap-13 pattern `useTopologyQuery().perProviderById ?? storeNode.per_provider` (NEVER the WS `topology_update` frame, which carries `per_provider` absent — deriving from flows would hide failed primaries); all flow-row tiles (volume/cost/failures via gap-14 `failureTaxonomy`, clients via gap-15 `clientRollup`, context via gap-09 `aggregateContextPressure`, token-mix) read the `useFlowRows(filters)` FLOW-LIST population (NOT the live `flow_status` frame); the headline echoes the live `metrics` store (gap-01 tile) + a LIVE-only `$/min` `useMetricStream` trend. The flow filter applies, so a topology/sankey/client cross-link re-scopes the WHOLE overview. Every tile is DQ-tagged + don't-lie-with-zeros (empty window ⇒ `—` empty-state, not an all-`0` board). When a future overview tile needs per-provider data, REUSE the gap-13 REST-path read — adding `per_provider` to the WS frame would be a NEW backend gap.
