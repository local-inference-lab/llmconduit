...
async fn uses_configured_upstream_model_override() {
    ...
    let gateway = test_gateway_with_config(
        ...
        Config {
            ...
            brave_api_key: None,
            brave_max_results: 5,
            kagi_base_url: "https://kagi.com/api/v1".parse().expect("url"),
            kagi_api_key: None,
            kagi_max_results: 5,
            search_backend: "brave".to_string(),
            searxng_base_url: "http://localhost:4040".parse().expect("url"),
            crawl4ai_base_url: "http://localhost:11235".parse().expect("url"),
...
async fn request_values_override_configured_upstream_defaults_and_merge_chat_template_kwargs() {
    ...
    // max_tokens is intentionally force-overridden by configured upstream defaults;
    // client max_output_tokens must not displace the gateway cap.
    assert_eq!(requests[0].max_output_tokens, None);
    assert_eq!(requests[0].extra_body.get("temperature"), None);
    assert_eq!(requests[0].extra_body.get("max_tokens"), Some(&json!(1024)));
